...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      fetch('https://dacexy-backend-v7ku.onrender.com/api/v1/auth/me', {
        headers: { Authorization: `Bearer ${token}` }
      }).then(r => r.json()).then(async userData => {
        let orgData = null
        try {
          const orgRes = await fetch('https://dacexy-backend-v7ku.onrender.com/api/v1/orgs/me', {
            headers: { Authorization: `Bearer ${token}` }
          })
  ...
  async function handleRegister(e?: React.FormEvent) {
    ...
    try {
      ...
      try {
        orgData = await fetch('https://dacexy-backend-v7ku.onrender.com/api/v1/orgs/me', {
  ...
  return (
    <div className="min-h-screen bg-[#F9F7F2] flex items-center justify-center p-6">
      <div className="bg-white rounded-3xl shadow-soft border border-black/6 p-10 w-full max-w-md">
        ...
        <a
          href="https://dacexy-backend-v7ku.onrender.com/api/v1/auth/google/login"
          className="w-full flex items-center justify-center gap-3 bg-white border border-black/10 rounded-xl px-4 py-3 text-sm font-semibold text-[#0F0F0F] hover:bg-gray-50 transition-all mb-5"
        >
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-xs text-[#9E9E9E]">or sign up with email</span>
          <div className="flex-1 h-px bg-black/6" />
        </div>
